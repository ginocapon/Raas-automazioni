...
3. **Implementazione** — Aggiungere una voce in `FONTI` con `id`, `nome`, `url`, `tipo` (`json`|`html`|`csv`|`rss`), `regione`, `ente`, `tipo_ente`. Seguire i parser esistenti in `index.ts` (`parseHtml`, `parseIncentiviGov`, …).
